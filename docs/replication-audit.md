...
Canvas 状态需以当前实际路由使用的 `LiveCanvas` 为准：它具有基础节点拖动、点选连线、缩放和撤销重做，保存会写入一条 SQLite 资源记录；但尚无完整节点配置、可靠的重载恢复、版本管理、路径校验或服务端执行追踪。旧的 `CanvasPage` 是未使用的简化组件，不能据此判断实际页面。Playwright 脚本、参考截图和截图差异验收没有实际落地。多数二级页继续复用通用资源列表。Vercel 的 SQLite 位于临时目录，不保证跨实例持久化；本地 SQLite 可持久化。

其他渠道与功能模块的逐项复核见[渠道与模块审计](channel-module-audit-2026-09-17.md)。

2026-09-17 后续修正：Canvas 已补固定草稿恢复、节点配置/图校验和持久执行路径；Performance Overview 路由及 Demo Lab 回执联动已修复。上文关于这些功能的缺口属于修正前状态，当前限制和验证证据以[渠道与模块审计的后续修正](channel-module-audit-2026-09-17.md#后续修正与实测2026-09-17)为准。
