...
function PageContent({ locale, page, campaigns, openPage, onEdit, onCreate, onStop, onArchive, onDuplicate, notify }: { locale: Locale; page: string; campaigns: Campaign[]; openPage: (p: string) => void; onEdit: (c: Campaign) => void; onCreate: (event: ReactMouseEvent<HTMLButtonElement>) => void; onStop: (id: string) => void; onArchive: (id: string) => Promise<boolean>; onDuplicate: (campaign: Campaign) => Promise<void>; notify: (m: string) => void }) {
  if (page === "campaigns") return <CampaignList locale={locale} campaigns={campaigns} onEdit={onEdit} onCreate={onCreate} onStop={onStop} onArchive={onArchive} onDuplicate={onDuplicate}/>;
...
function CampaignList({ locale, campaigns, onEdit, onCreate, onStop, onArchive, onDuplicate }: { locale: Locale; campaigns: Campaign[]; onEdit: (c: Campaign) => void; onCreate: (event: ReactMouseEvent<HTMLButtonElement>) => void; onStop: (id: string) => void; onArchive: (id: string) => Promise<boolean>; onDuplicate: (campaign: Campaign) => Promise<void> }) {
  ...
  return <section className="page-content"><div className="page-heading"><div><div className="title-line"><h1>{translate(locale, "Campaigns")}</h1><span className="access-pill">{translate(locale, "Limited access")}</span></div><p>{translate(locale, "Campaigns let you send a single, targeted message through email, push, SMS, and more, ensuring timely communication with your audience")}</p></div><div className="heading-actions"><button className="secondary">{translate(locale, "Send feedback")}</button><button className="secondary">{translate(locale, "Take a tour")} <ChevronDown size={14}/></button><button className="primary" onClick={onCreate}><Plus size={16}/> {translate(locale, "Create campaign")} <ChevronDown size={14}/></button></div></div><div className="filters"><label>{translate(locale, "Status")}<select value={query.status} onChange={e => changeQuery({ status: e.target.value, start: 0 })}><option value="All">{statusLabel("All")}</option><option value="Draft">{statusLabel("Draft")}</option><option value="Active">{statusLabel("Active")}</option><option value="Stopped">{statusLabel("Stopped")}</option></select></label><label>{translate(locale, "Tag")}<select><option>{translate(locale, "Select...")}</option><option>Lifecycle</option><option>Promotional</option></select></label><button className="secondary"><Filter size={15}/> {translate(locale, "Filters")}</button><button className="secondary"><Grid2X2 size={15}/> {translate(locale, "Columns")}</button><button className="text-button" onClick={() => changeQuery({ status: "All", channel: "all", search: "", start: 0 })}>{translate(locale, "Reset filters")}</button><div className="filter-search"><Search size={15}/><input aria-label={translate(locale, "Search")} placeholder={translate(locale, "Search")} value={query.search} onChange={e => changeQuery({ search: e.target.value, start: 0 }, true)}/></div></div><div className="result-heading"><span>{formatNumber(locale, filtered.length)} {translate(locale, "Results")}</span><small>{query.channel !== "all" ? translate(locale, channelMeta[query.channel].title) : query.status !== "All" ? `${translate(locale, "Status")}: ${statusLabel(query.status)}` : translate(locale, "All campaigns")}</small></div><div className="table-wrap"><table><thead><tr><th onClick={() => changeQuery({ sort: "name", direction: query.sort === "name" && query.direction === 1 ? -1 : 1, start: 0 })}>{translate(locale, "Name")} {query.sort === "name" ? query.direction === 1 ? "↑" : "↓" : ""}</th><th>{translate(locale, "Status")}</th><th>{translate(locale, "Stop date")}</th><th>{translate(locale, "Campaign type")}</th><th>{translate(locale, "Entry schedule")}</th><th>{translate(locale, "Sent")}</th><th>{translate(locale, "Last edited")}</th><th/></tr></thead><tbody>{paged.map(c => { const Icon = channelMeta[c.channel].icon; return <tr key={c.id}><td><button className="link-button" onClick={() => onEdit(c)}>{c.name}</button></td><td><span className={className("status", c.status.toLowerCase())}>{statusLabel(c.status)}</span></td><td>—</td><td><span className="channel-cell"><Icon size={14}/>{translate(locale, channelMeta[c.channel].title)}</span></td><td>{translate(locale, c.schedule)}</td><td>{formatNumber(locale, c.sent)}</td><td>{formatDate(locale, c.edited, { dateStyle: "medium", timeStyle: "short" })}</td><td><CampaignRowActions locale={locale} campaign={c} onEdit={onEdit} onStop={onStop} onArchive={onArchive} onDuplicate={onDuplicate}/></td></tr>; })}</tbody></table></div><div className="campaign-pagination"><span>{formatNumber(locale, filtered.length ? start + 1 : 0)}–{formatNumber(locale, Math.min(start + query.limit, filtered.length))} / {formatNumber(locale, filtered.length)}</span><label>{translate(locale, "Rows per page")} <select value={query.limit} onChange={e => changeQuery({ limit: Number(e.target.value), start: 0 })}><option value="12">12</option><option value="24">24</option><option value="48">48</option></select></label><button className="secondary small" disabled={start === 0} onClick={() => changeQuery({ start: Math.max(0, start - query.limit) })} aria-label={translate(locale, "Previous page")}><ChevronLeft size={15}/></button><button className="secondary small" disabled={start + query.limit >= filtered.length} onClick={() => changeQuery({ start: start + query.limit })} aria-label={translate(locale, "Next page")}><ChevronRight size={15}/></button></div></section>;
}
...
function EmailCompose({ locale, draft, update, saveEmail, variant, setVariant, openTest }: { locale: Locale; draft: Campaign; update: (p: Partial<Campaign>) => void; saveEmail: (p: Partial<Campaign>) => Promise<boolean>; variant: number; setVariant: (x: number) => void; openTest: () => void }) {
  ...
  if (emailEditor) return <EmailMessageEditor locale={locale} key={current.id} mode={emailEditor} variantName={current.name} draft={variantDraft} save={saveVariant} onClose={() => { setEmailEditor(null); setEmailStart(false); }} onSendingSettings={() => { setEmailEditor(null); setEmailStart(false); setShowSending(true); }} />;
  ...
  return <div className="editor-body email-compose-page">
    ...
    <section className="braze-section email-composer-section">
      ...
      <div className="email-body-heading"><div><h3>{translate(locale, "Email body")}</h3><p>{savedEditorMode === "drag" ? "Drag-and-drop Editor" : "HTML Editor"}</p></div><button className="secondary" onClick={() => setEmailEditor(savedEditorMode ?? "html")}>{translate(locale, "Edit message")}</button></div>
      ...
      <div className="email-actions"><button className="secondary" onClick={() => setShowTemplates(!showTemplates)}>{translate(locale, "Choose New Template")}</button><button className="secondary" onClick={openTest}>{translate(locale, "Preview and test")}</button></div>
      {showTemplates && <div className="template-picker"><b>{translate(locale, "Choose a template")}</b><button onClick={() => { updateVariant({subject:"Welcome to Braze"}); setShowTemplates(false); }}>Welcome email <small>Drag-and-Drop Editor</small></button><button onClick={() => { updateVariant({subject:"Your September Offer | 20% Off"}); setShowTemplates(false); }}>September Offer <small>HTML Editor</small></button></div>}
    </section>
  </div>;
}
...
function EmailStart({ locale, draft, update, importHtml, variants, variant, setVariant, addVariant, copied, setCopied, onStart }: { locale: Locale; draft: Campaign; update: (patch: Partial<Campaign>) => void; importHtml: (body: string) => void; variants: EmailVariant[]; variant: number; setVariant: (variant: number) => void; addVariant: () => void; copied: boolean; setCopied: (copied: boolean) => void; onStart: (choice: "operator" | "drag" | "html" | "template") => void }) {
  return <div className="editor-body email-compose-page">
    <EmailCampaignDetails locale={locale} draft={draft} update={update} copied={copied} setCopied={setCopied}/>
    <section className="braze-section email-composer-section">
      <h2>{translate(locale, "Email Composer")}</h2>
...
function EmailSendingEditor({ locale, draft, sending, save, onClose }: { locale: Locale; draft: Campaign; sending?: EmailSending; save: (patch: Partial<Campaign>) => Promise<boolean>; onClose: () => void }) {
  ...
  return <section className={brazeSendingStyles.page} aria-label={translate(locale, "Email sending settings")}>
    ...
    <nav className={brazeSendingStyles.iconRail} aria-label={translate(locale, "Email editor sections")}><button className={brazeSendingStyles.iconActive} title={translate(locale, "Sending settings")}><Mail size={22}/></button><button title={translate(locale, "Content")} onClick={onClose}><FileCode2 size={22}/></button><button title={translate(locale, "Preview & Test")} onClick={() => setSection("Sending Info")}><Eye size={22}/></button></nav>
...
function EmailMessageEditor({ locale, mode, variantName, draft, save, onClose, onSendingSettings }: { locale: Locale; mode: EmailEditorMode; variantName: string; draft: Campaign; save: (patch: Partial<Campaign>) => Promise<boolean>; onClose: () => void; onSendingSettings: () => void }) {
  ...
  if (mode === "drag") return <BrazeDragDropEditor locale={locale} draft={draft} variantName={variantName} save={save} onClose={onClose} onSendingSettings={onSendingSettings}/>;
  return <section className={emailEditorStyles.page} aria-label="Email editor">
    <header className={emailEditorStyles.header}>
      <button className={emailEditorStyles.back} onClick={saveAndReturn}><ChevronLeft size={17}/> {translate(locale, "Back to campaign")}</button>
      <div className={emailEditorStyles.title}><b>{translate(locale, labels[mode])}</b><small>{draft.name} · {variantName}</small></div>
      <div className={emailEditorStyles.headerActions}><button className={emailEditorStyles.previewButton}><Smartphone size={15}/> {translate(locale, "Preview")}</button><button className={emailEditorStyles.previewButton} onClick={() => void openSendingSettings()}><Mail size={15}/> {translate(locale, "Sending settings")}</button><button className={emailEditorStyles.saveButton} onClick={saveAndReturn}><Send size={14}/> {translate(locale, "Save")}</button></div>
    </header>
    <div className={emailEditorStyles.workspace}>
      <aside className={emailEditorStyles.sidebar}>
        {mode === "html" && <><h2>{translate(locale, "HTML code")}</h2><p>Edit your email source. The central canvas renders its current message content.</p><textarea className={emailEditorStyles.codeArea} value={body} onChange={event => setBody(event.target.value)} aria-label="Email HTML source"/><div className={emailEditorStyles.personalizationTools}><button type="button" onClick={() => setPersonalizationOpen(value => !value)}><Sparkles size={14}/> {translate(locale, "Personalization")}</button>{personalizationOpen && <PersonalizationPicker title="Insert into HTML" onInsert={insertToken}/>}</div></>}
        {mode === "template" && <><h2>{translate(locale, "Templates")}</h2><p>Choose a template, then make changes in the settings panel.</p><div className={emailEditorStyles.templateList}><button className={`${emailEditorStyles.template} ${emailEditorStyles.templateActive}`} onClick={() => applyTemplate("Your September Offer | 20% Off", "Thanks for being with us. Use code SEPTEMBER20 to get 20% off this month’s featured collection.")}><b>September Offer</b><small>HTML Editor</small></button><button className={emailEditorStyles.template} onClick={() => applyTemplate("Welcome to Braze", "We’re glad you’re here. Explore the latest ways to make every customer interaction count.")}><b>Welcome email</b><small>Drag-and-drop Editor</small></button></div></>}
        {mode === "operator" && <><h2>{translate(locale, "Operator")}</h2><p>Describe the email you want to create.</p><textarea className={emailEditorStyles.operatorPrompt} value={operatorPrompt} onChange={event => setOperatorPrompt(event.target.value)}/><button className={emailEditorStyles.generate} onClick={() => applyTemplate("Your first order offer", "Welcome! Use code WELCOME10 for 10% off your first purchase. This offer is ready whenever you are.")}><Sparkles size={14}/> {translate(locale, "Generate email")}</button></>}
      </aside>
      <main className={emailEditorStyles.canvas}>
        <div className={emailEditorStyles.canvasLabel}>{translate(locale, "Email preview")} · 600 px</div>
      ...
      <aside className={emailEditorStyles.settings}>
        <h2>{translate(locale, "Message settings")}</h2><label className={emailEditorStyles.field}><span>{translate(locale, "Subject line")}</span><input value={subject} onChange={event => setSubject(event.target.value)}/></label><label className={emailEditorStyles.field}><span>{translate(locale, "Preheader")}</span><input value={preheader} onChange={event => setPreheader(event.target.value)}/></label><label className={emailEditorStyles.field}><span>{translate(locale, "Message")}</span><textarea value={body} onChange={event => setBody(event.target.value)}/></label><div className={emailEditorStyles.personalizationTools}><button type="button" onClick={() => setPersonalizationOpen(value => !value)}><Sparkles size={14}/> {translate(locale, "Personalization")}</button>{personalizationOpen && <PersonalizationPicker title="Insert into message" onInsert={insertToken}/>}</div><div className={emailEditorStyles.note}>Personalization, content blocks, and link tracking are available in this local editor.</div>
      </aside>
    </div>
  </section>;
}

function BrazeDragDropEditor({ locale, draft, variantName, save: saveCampaignMessage, onClose, onSendingSettings }: { locale: Locale; draft: Campaign; variantName: string; save: (patch: Partial<Campaign>) => Promise<boolean>; onClose: () => void; onSendingSettings: () => void }) {
  ...
  return <section className={brazeDndStyles.page} aria-label={translate(locale, "Braze drag-and-drop email editor")}>
    <header className={brazeDndStyles.header}><b>{draft.name}</b><button aria-label="BrazeAI Operator"><Sparkles size={19}/></button></header>
    <nav className={brazeDndStyles.iconRail} aria-label="Email editor sections"><button title={translate(locale, "Sending settings")} onClick={() => void openSendingSettings()}><Mail size={22}/></button><button className={brazeDndStyles.iconActive} title={translate(locale, "Content")}><FileCode2 size={22}/></button><button title={translate(locale, "Preview & Test")} onClick={() => setRightTab("settings")}><Eye size={22}/></button></nav>
    <aside className={brazeDndStyles.leftRail}>
      <div className={brazeDndStyles.leftTitle}>{translate(locale, "CONTENT")}<button aria-label={translate(locale, "Back to campaign")} onClick={() => void save()}><ChevronLeft size={16}/></button></div>
      <button className={leftMode === "content" ? brazeDndStyles.leftActive : ""} onClick={() => setLeftMode("content")}>{translate(locale, "Content Build")}</button>
      <button className={leftMode === "links" ? brazeDndStyles.leftActive : ""} onClick={() => setLeftMode("links")}>{translate(locale, "Link Management")}</button>
      <button className={leftMode === "personalization" ? brazeDndStyles.leftActive : ""} onClick={() => setLeftMode("personalization")}>✚ &nbsp; {translate(locale, "Personalization")}</button>
      <button className={leftMode === "languages" ? brazeDndStyles.leftActive : ""} onClick={() => setLeftMode("languages")}>◉ &nbsp; {translate(locale, "Languages")}</button>
      <small>{translate(locale, "Create with Operator")}</small><button onClick={() => { if (selectedBlock) changeSelected("A fresh offer, just for you"); else add("Title"); }}>✎ &nbsp; {translate(locale, "Copy")}</button>
      <button className={brazeDndStyles.styleLink} onClick={() => { setSelected(null); setRightTab("settings"); }}>{translate(locale, "Style Settings")} ↗</button>
    </aside>
    ...
    <aside className={brazeDndStyles.rightPanel}>
      ...
      {selectedBlock ? <div className={brazeDndStyles.properties}>
        ...
        <h3>{translate(locale, "BASIC BLOCKS")}</h3><div className={brazeDndStyles.blockCatalog}>{catalog.slice(0,6).map(block => { const Icon = block.kind === "Title" ? Type : block.kind === "Paragraph" ? FileCode2 : block.kind === "List" ? List : block.kind === "Button" ? MousePointerClick : block.kind === "Divider" ? Grid2X2 : LayoutDashboard; return <button draggable key={block.kind} onDragStart={event => event.dataTransfer.setData("braze-block", block.kind)} onClick={() => add(block.kind)}><Icon size={29}/><span>{translate(locale, block.kind.toUpperCase())}</span></button>; })}</div>
        <h3>{translate(locale, "MEDIA")}</h3><div className={brazeDndStyles.blockCatalog}>{catalog.slice(6,9).map(block => { const Icon = block.kind === "Image" ? Image : block.kind === "Video" ? Video : Share2; return <button draggable key={block.kind} onDragStart={event => event.dataTransfer.setData("braze-block", block.kind)} onClick={() => add(block.kind)}><Icon size={29}/><span>{translate(locale, block.kind.toUpperCase())}</span></button>; })}</div>
        <h3>{translate(locale, "ADVANCED")}</h3><div className={brazeDndStyles.blockCatalog}>{catalog.slice(9).map(block => <button draggable key={block.kind} onDragStart={event => event.dataTransfer.setData("braze-block", block.kind)} onClick={() => add(block.kind)}><Code2 size={29}/><span>{translate(locale, block.kind.toUpperCase())}</span></button>)}</div>
      </div> : rightTab === "rows" ? <div className={brazeDndStyles.catalogPane}><h3>{translate(locale, "ROWS")}</h3><p>Drag a layout into your email.</p><div className={brazeDndStyles.rowCatalog}>{[1,2,3].map(columns => <button key={columns} onClick={() => { for (let i = 0; i < columns; i++) add("Paragraph"); }}>{columns} column{columns > 1 ? "s" : ""}<span>{Array.from({ length: columns }, (_, index) => <i key={index}/>)}</span></button>)}</div></div> : <div className={brazeDndStyles.catalogPane}><h3>{translate(locale, "SETTINGS")}</h3><label className={brazeDndStyles.settingField}>Background color<input type="color" value={canvasBg} onChange={event => setCanvasBg(event.target.value)}/></label><label className={brazeDndStyles.settingField}>Content width <input type="number" min="320" max="800" value={contentWidth} onChange={event => setContentWidth(Math.min(800, Math.max(320, Number(event.target.value) || 600)))}/></label></div>}
    ...
    <footer className={brazeDndStyles.footer}><button onClick={download}>{translate(locale, "Download file")}</button><button className={brazeDndStyles.done} onClick={() => void save()}>{translate(locale, "Done")}</button></footer>
  </section>
}
...
function ChannelFields({ draft, update, openTest = () => {} }: { draft: Campaign; update: (patch: Partial<Campaign>) => void; openTest?: () => void }) {
  const channel = draft.channel;
  ...
  if (channel === "whatsapp") return <WhatsAppEditor draft={draft} update={update}/>;
...
function Audience({ locale, draft, update }: { locale: Locale; draft: Campaign; update: (p: Partial<Campaign>) => void }) {
  ...
  return <div className="editor-body braze-flow"><section className="editor-card"><h2>{translate(locale, "Targeting Options")}</h2><p>{translate(locale, "Target users by choosing multiple segments they must fall into. Further refine your audience by adding additional filters.")}</p>
    ...
    <div className="audience-summary"><h3>{translate(locale, "Audience Summary")}</h3><p>{audienceSummary(draft)}</p></div><div className="audience-summary"><h3>{translate(locale, "User Lookup")}</h3><p>{translate(locale, "Check if a user matches the segment, filter, and app criteria")}</p><button className="secondary small" onClick={() => document.getElementById("audience-user-lookup")?.focus()}>{translate(locale, "Lookup User")}</button><input id="audience-user-lookup" placeholder={translate(locale, "Search user ID")}/></div>
...
function TestModal({ draft, close }: { draft: Campaign; close: () => void }) {
  const webhookVariants = draft.channel === "webhook" ? normalizeWebhookVariants(draft) : [];
  const whatsappVariants = draft.channel === "whatsapp" ? normalizeWhatsAppVariants(draft) : [];
  ...
  const [variantId, setVariantId] = useState(webhookVariants[0]?.id ?? whatsappVariants[0]?.id ?? "");
  ...
  const send = async () => {
    ...
    const response = await fetch(`/api/campaigns/${draft.id}/test`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ recipient, mode, customUser: parsedCustom, variantId, locale }) });
    const payload = await response.json().catch(() => ({}));
    setResult(payload); setError(response.ok ? "" : String(payload.error ?? "The test request failed.")); setState(response.ok ? "sent" : "error");
  };
  ...
  return <div className="modal-backdrop"><section className="test-modal webhook-test-modal"><button className="modal-close" onClick={close}><X size={18}/></button><h2>Preview and test</h2><p>{draft.channel === "webhook" ? "Send a real request through the protected webhook delivery service and inspect the endpoint response." : draft.channel === "whatsapp" ? "Simulate a WhatsApp test delivery in the local activity log. No Meta API or device is contacted." : "Write a test delivery event to the local SQLite activity log."}</p>{draft.channel === "webhook" && <><Field label="Preview as"><select value={mode} onChange={event => setMode(event.target.value as typeof mode)}><option value="random">Random user</option><option value="existing">Existing user</option><option value="custom">Custom user</option></select></Field>{mode === "existing" && <Field label="Email or external user ID"><input value={recipient} onChange={event => setRecipient(event.target.value)}/></Field>}{mode === "custom" && <Field label="Custom user attributes"><textarea aria-label="Custom user attributes" value={customUser} onChange={event => setCustomUser(event.target.value)}/></Field>}<div className="form-grid"><Field label="Variant"><select value={variantId} onChange={event => setVariantId(event.target.value)}>{webhookVariants.map(variant => <option value={variant.id} key={variant.id}>{variant.name}</option>)}</select></Field><Field label="Locale"><input value={locale} onChange={event => setLocale(event.target.value)}/></Field></div></>}{draft.channel === "whatsapp" && <><Field label="WhatsApp phone number"><input value={recipient} onChange={event => setRecipient(event.target.value)}/></Field><Field label="Variant"><select value={variantId} onChange={event => setVariantId(event.target.value)}>{whatsappVariants.map(variant => <option value={variant.id} key={variant.id}>{variant.name} · {variant.mode === "template" ? getWhatsAppTemplate(variant.templateId).name : "Response message"}</option>)}</select></Field><div className="review-email-preview"><small>Rendered preview</small><div>{renderWhatsAppVariant(whatsappVariants.find(variant => variant.id === variantId) ?? whatsappVariants[0])}</div></div></>}{draft.channel !== "webhook" && draft.channel !== "whatsapp" && <Field label="Test user"><input value={recipient} onChange={event => setRecipient(event.target.value)}/></Field>}<button className="primary" disabled={state === "sending"} onClick={send}><Send size={15}/>{state === "sending" ? "Sending…" : draft.channel === "whatsapp" ? "Record simulated test" : "Send test"}</button>{state === "sent" && draft.channel !== "webhook" && <div className="test-success"><ShieldCheck size={18}/><span>{draft.channel === "whatsapp" ? "Simulated WhatsApp test recorded" : "Test delivery recorded"} for {recipient}.</span></div>}{state === "sent" && webhook && <div className={webhook.delivered ? "test-success webhook-test-result" : "review-issues webhook-test-result"}><b>{attempt?.statusCode ? `${attempt.statusCode} ${attempt.outcome}` : attempt?.outcome ?? "failed"}</b><span>{webhook.method} {webhook.url}</span><small>{attempt?.durationMs ?? 0} ms</small>{attempt?.responseBody && <pre>{attempt.responseBody}</pre>}{attempt?.error && <pre>{attempt.error}</pre>}</div>}{state === "error" && <p className="form-error">{error || "The test request failed."}</p>}</section></div>;
}
...
function LiveReportPage({ locale, title }: { locale: Locale; title: string }) {
  ...
  return <section className="page-content"><div className="page-heading"><div><h1>{translate(locale, title)}</h1><p>{translate(locale, "Metrics are calculated from local execution runs and message events.")}</p></div><select className="date-select" value={range} onChange={event => setRange(event.target.value)}><option value="7">{translate(locale, "Last 7 days")}</option><option value="30">{translate(locale, "Last 30 days")}</option><option value="90">{translate(locale, "Last 90 days")}</option></select></div><div className="metric-grid">{[["Delivered", delivered], ["Opened", opened], ["Clicked", clicked], ["Suppressed", overview?.suppressed ?? 0], ["Unreachable", overview?.unreachable ?? 0]].map(([label, value]) => <div className="metric" key={String(label)}><small>{translate(locale, String(label))}</small><b>{formatNumber(locale, Number(value))}</b><span>{delivered ? `${Math.round(Number(value) / delivered * 100)}% ${translate(locale, "of delivered")}` : translate(locale, "No executions yet")}</span></div>)}</div><section className="chart-card"><div><h2>{translate(locale, "Delivery runs")}</h2><p>{translate(locale, "Each bar is a completed local Campaign or Canvas execution snapshot.")}</p></div><div className="bar-chart" aria-label={translate(locale, "Delivery run chart")}>{overview?.series.length ? overview.series.map((point, index) => <span key={`${point.date}-${index}`} title={`${formatNumber(locale, point.delivered)} ${translate(locale, "Delivered").toLowerCase()}`} style={{ height: `${Math.max(8, point.delivered / max * 100)}%` }}/>) : <p className="empty-inline">{translate(locale, "Launch a Campaign or Canvas to create report data.")}</p>}</div></section></section>;
}
...
function ActivityLogPage({ locale }: { locale: Locale }) {
  ...
  return <section className="page-content"><div className="page-heading"><div><h1>{translate(locale, "Message Activity Log")}</h1><p>{translate(locale, "Local test, delivery, engagement, suppression and failure events.")}</p></div></div><div className="table-wrap"><table><thead><tr><th>{translate(locale, "Event")}</th><th>{translate(locale, "Campaign")}</th><th>{translate(locale, "Channel")}</th><th>{translate(locale, "User")}</th><th>Response</th><th>{translate(locale, "Timestamp")}</th></tr></thead><tbody>{events.length ? events.map(event => <tr key={String(event.id)}><td><span className={className("status", String(event.event_type) === "failed" ? "stopped" : "active")}>{translate(locale, String(event.event_type))}</span></td><td>{String(event.campaign_name ?? event.campaign_id)}</td><td>{translate(locale, String(event.channel))}</td><td>{String(event.user_id)}</td><td className="activity-detail">{detail(event)}</td><td>{formatDate(locale, String(event.created_at), { dateStyle: "medium", timeStyle: "short" })}</td></tr>) : <tr><td colSpan={6}>{translate(locale, "No message events have been recorded.")}</td></tr>}</tbody></table></div></section>;
}
...
function PersistentResourceList({ locale, page, title, notify, creating, setCreating }: { locale: Locale; page: string; title: string; notify: (message: string) => void; creating: boolean; setCreating: (value: boolean) => void }) {
  ...
  return <section className="resource-workspace"><div className="list-toolbar"><div className="filter-search"><Search size={15}/><input aria-label={translate(locale, "Search")} placeholder={`${translate(locale, "Search")} ${translate(locale, title)}`} value={query} onChange={event => setQuery(event.target.value)}/></div><button className="secondary" onClick={() => setCreating(!creating)}><Plus size={15}/> {translate(locale, "Create")}</button></div>{creating && <div className="resource-create"><input id="resource-name" autoFocus placeholder={`${translate(locale, "Name")} · ${translate(locale, singular(title))}`} value={name} onChange={event => setName(event.target.value)} onKeyDown={event => { if (event.key === "Enter") void create(); }}/><button className="primary small" onClick={() => void create()}>{translate(locale, "Save")}</button></div>}<div className="resource-cards">{rows.length ? rows.map(row => <article key={row.id}><div className="resource-avatar">{row.name[0]}</div><div><b>{row.name}</b><p>{row.description}</p><small>{translate(locale, row.status)} · {formatDate(locale, row.updatedAt, { dateStyle: "medium", timeStyle: "short" })}</small></div><button className="icon-button" aria-label={translate(locale, "Actions")} onClick={() => notify(`${row.name} opened in its local workspace.`)}><MoreHorizontal size={18}/></button></article>) : <div className="empty-inline">{translate(locale, "No results found")}</div>}</div></section>;
}
