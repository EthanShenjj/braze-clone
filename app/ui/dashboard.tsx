...
function CampaignEditor({ locale, campaign, onSave, onClose }: { locale: Locale; campaign: Campaign; onSave: (c: Campaign, publish?: boolean) => Promise<boolean>; onClose: () => void }) {
  ...
  return <section className="editor-page">
    ...
    {step === 0 && <Compose locale={locale} draft={draft} update={update} saveEmail={saveMessage} variant={variant} setVariant={setVariant} openTest={() => setTestOpen(true)}/>}
...
function ActivityLogPage({ locale }: { locale: Locale }) {
  ...
  return <section className="page-content"><div className="page-heading"><div><h1>{translate(locale, "Message Activity Log")}</h1><p>{translate(locale, "Local test, delivery, engagement, suppression and failure events.")}</p></div></div><div className="table-wrap"><table><thead><tr><th>{translate(locale, "Event")}</th><th>{translate(locale, "Campaign")}</th><th>{translate(locale, "Channel")}</th><th>{translate(locale, "User")}</th><th>{translate(locale, "Timestamp")}</th></tr></thead><tbody>{events.length ? events.map(event => <tr key={String(event.id)}><td><span className="status active">{translate(locale, String(event.event_type))}</span></td><td>{String(event.campaign_name ?? event.campaign_id)}</td><td>{translate(locale, String(event.channel))}</td><td>{String(event.user_id)}</td><td>{formatDate(locale, String(event.created_at), { dateStyle: "medium", timeStyle: "short" })}</td></tr>) : <tr><td colSpan={5}>{translate(locale, "No message events have been recorded.")}</td></tr>}</tbody></table></div></section>;
}
