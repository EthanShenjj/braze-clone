...
export default function LiveCanvas({ notify }: { notify: (message: string) => void }) {
  ...
  return <section className="canvas-page"><div className="page-heading"><div><h1>Canvas</h1><p>Build, validate, save, and simulate a local customer journey.</p></div><div className="heading-actions"><button className="secondary" disabled={cursor === 0 || working} onClick={() => setCursor(cursor - 1)}>Undo</button><button className="secondary" disabled={cursor === history.length - 1 || working} onClick={() => setCursor(cursor + 1)}>Redo</button><button className="secondary" disabled={working} onClick={() => void save()}>Save Draft</button><button className="primary" disabled={working} onClick={() => void launch()}>Launch Canvas</button></div></div>
    {error && <div className="canvas-error" role="alert">{error}</div>}
    <div className="canvas-shell live-canvas-shell"><aside><h3>Steps</h3>{(["entry", "message", "delay", "branch", "update"] as CanvasNodeKind[]).map(kind => <button key={kind} onClick={() => add(kind)}><Plus size={14}/>{kind === "entry" ? "Audience Paths" : kind === "message" ? "Message" : kind === "delay" ? "Delay" : kind === "branch" ? "Action Paths" : "Update User"}</button>)}
      <hr/><h3>Inspector</h3>{selectedNode ? <><label>Step name<input value={selectedNode.label} onChange={event => changeNode(selected, { label: event.target.value })}/></label>
        {selectedNode.kind === "delay" && <label>Delay (hours)<input type="number" min="1" value={selectedNode.config?.durationHours ?? 24} onChange={event => changeConfig(selected, { durationHours: Number(event.target.value) })}/></label>}
        {selectedNode.kind === "message" && <><label>Channel<select value={selectedNode.config?.channel ?? "email"} onChange={event => changeConfig(selected, { channel: event.target.value })}><option value="email">Email</option><option value="push">Push notification</option><option value="sms">SMS</option><option value="iam">In-app message</option></select></label><label>Message<textarea value={selectedNode.config?.message ?? ""} onChange={event => changeConfig(selected, { message: event.target.value })}/></label></>}
        {selectedNode.kind === "branch" && <><label>Attribute<select value={selectedNode.config?.attribute ?? "country"} onChange={event => changeConfig(selected, { attribute: event.target.value })}><option value="country">Country</option><option value="lifecycle">Lifecycle</option><option value="subscribed">Subscribed</option></select></label><label>Equals<input value={selectedNode.config?.value ?? ""} onChange={event => changeConfig(selected, { value: event.target.value })}/></label><small>First outgoing route is Yes; second is No.</small></>}
        <button className="secondary small" onClick={() => setLinkFrom(selected)}>Connect from this step</button><button className="secondary small" onClick={copy}>Copy step</button><button className="text-button danger" onClick={remove}>Delete step</button></> : <p>Select a node</p>}
      <hr/><h3>Execution</h3>{runs.length ? <><label>Run<select value={run?.id ?? ""} onChange={event => { const next = runs.find(item => item.id === event.target.value); setRunId(next?.id ?? null); setTraceUser(next?.traces[0]?.userId ?? null); }}>{runs.map(item => <option key={item.id} value={item.id}>{new Date(item.createdAt).toLocaleString()}</option>)}</select></label><small>{run?.entered} entered · {run?.completed} completed · {run?.messages} messages</small><label>User path<select value={trace?.userId ?? ""} onChange={event => setTraceUser(event.target.value)}>{run?.traces.map(item => <option key={item.userId}>{item.userId}</option>)}</select></label>{trace?.steps.map((step, index) => <small className="execution-item" key={`${step.nodeId}-${index}`}>✓ {step.label}</small>)}</> : <small>Launch to inspect the local path.</small>}</aside>
      <div className="journey-canvas live-journey-canvas"><div className="zoom-control"><button onClick={() => setZoom(value => Math.max(.6, value - .1))}>−</button><span>{Math.round(zoom * 100)}%</span><button onClick={() => setZoom(value => Math.min(1.5, value + .1))}>+</button></div><div className="canvas-world" style={{ transform: `scale(${zoom})` }}>{visible.edges.map(([from, to]) => { const start = visible.nodes.find(node => node.id === from); const end = visible.nodes.find(node => node.id === to); return start && end ? <svg className="canvas-edge" key={`${from}-${to}`}><line x1={start.x + 160} y1={start.y + 36} x2={end.x} y2={end.y + 36}/></svg> : null; })}{visible.nodes.map(node => <button key={node.id} className={["journey-node", selected === node.id ? "selected" : "", linkFrom === node.id ? "link-source" : ""].filter(Boolean).join(" ")} style={{ left: node.x, top: node.y }} onPointerDown={event => pointerDown(node.id, event)} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp} onClick={() => selectNode(node.id)}>{icon(node.kind)}<span>{node.label}</span></button>)}</div>{linkFrom && <div className="connect-hint">Select a destination step to create a connection.</div>}</div></div></section>;
}
