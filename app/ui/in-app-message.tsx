...
import {
  ChevronDown, ChevronLeft, ChevronRight, Code2, Copy, Eye, Image as ImageIcon,
...
function InAppEditor({ initial, close, done }: { initial: InAppVariant; close: () => void; done: (variant: InAppVariant) => void }) {
  ...
  const reorder = (index: number, direction: -1 | 1) => { const target = index + direction; if (target < 0 || target >= pages.length) return; const next = [...pages]; [next[index], next[target]] = [next[target], next[index]]; commitPages(next); setSelectedPage(target); };
...
function PagesPanel({ pages, selected, select, add, remove, reorder }: { pages: InAppPage[]; selected: number; select: (index: number) => void; add: () => void; remove: (index: number) => void; reorder: (index: number, direction: -1 | 1) => void }) {
  return <section className={styles.pagesPanel}><h3>Pages ({pages.length})</h3><p>Create and manage multi-page messages</p>{pages.map((page, index) => <div className={`${styles.pageRow} ${selected === index ? styles.pageSelected : ""}`} key={page.id}><button className={styles.pageMain} onClick={() => select(index)}><span>{index + 1}</span><b>{page.name}</b></button><button aria-label={`Move ${page.name} up`} disabled={index === 0} onClick={() => reorder(index, -1)}><ChevronLeft size={13}/></button><button aria-label={`Move ${page.name} down`} disabled={index === pages.length - 1} onClick={() => reorder(index, 1)}><ChevronRight size={13}/></button><button aria-label={`Delete ${page.name}`} disabled={pages.length === 1} onClick={() => remove(index)}><Trash2 size={13}/></button></div>)}<button className={styles.addPage} onClick={add}><Plus size={16}/> Add Page</button></section>;
}
