...
export async function POST(request: Request) {
  const input = await request.json() as { name?: unknown; description?: unknown; groupIds?: unknown; status?: unknown };
  ...
  try { return Response.json(createPreferenceCenter({ name: input.name, description: typeof input.description === "string" ? input.description : "", groupIds: input.groupIds.filter((value): value is string => typeof value === "string"), status: input.status === "Active" ? "Active" : "Draft" }), { status: 201 }); }
